...
internal class HilbertPoint : UnsignedPoint, IEquatable<HilbertPoint>, IComparable<HilbertPoint>
{
    ...
    {
        return Equals((UnsignedPoint)other);
    }

    public int CompareTo(HilbertPoint other)
    {
        int cmp = HilbertIndex.CompareTo(other.HilbertIndex);
        if (cmp == 0)
        {
    ...
    public override UnsignedPoint AppendCoordinate(uint coordinate)
    {
        ...
        if (limit <= coordinate)
        {
            throw new ArgumentOutOfRangeException("coordinate", $"Value must be smaller than 2^BitsPerDimension, which is {limit}");
        }
